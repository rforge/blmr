...
double Cblmr::geo(const double th2, double *const perr)  const
...
double Cblmr::geo_vu_NDab(const int k, const double th_a, const double th_b, const int hilo, double *const err)  const
...
//  'Rdqag' parameters
		...
		if( zero )  {	// mu = zero  on interval
			...
			if( aa > 7.5 )  tha= bisect( tha, thZ, &Cblmr::amu_by_Omega, k, 7, inc_x);			
			if( ab > 7.5  &&  !isinf(thb) )  thb= bisect( thZ, thb, &Cblmr::amu_by_Omega, k, 7, inc_x);			
...
//Rcout << "tha thZ pr  " << tha << " " << thZ << " " << pr << endl;
			...
			if(aa>7.5 && ab<6.5)  {
				if( !isinf(thb) )  
					tha= bisect( tha, thb, &Cblmr::amu_by_Omega, k, 7, inc_x);
				else  {
					double th= min( -1., tha);
					while( amu_by_Omega(th,k) > 6.8 )  th *= 2;
					tha= bisect( tha, th, &Cblmr::amu_by_Omega, k, 7, inc_x);
				}
			}
			if(aa<6.5 && ab>7.5 && !isinf(thb) )  thb= bisect( tha, thb, &Cblmr::amu_by_Omega, k, 7, inc_x);
...
double Cblmr::geo_vk_NDab( const int k, const double th_a, const double th_b, const int hilo, double *const err )  const 
...
{
	const Cblmr  **const  ppObj  = static_cast< const Cblmr **const > ( ex );
	const Cblmr  *const  pObj = ppObj[0];
