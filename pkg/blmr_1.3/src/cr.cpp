...
// store boundary values in an  N x 3  array	
	...
	double min_th = max(th_bds[0],Xs[1]), max_th = min(th_bds[2*numr-1],Xs[n-2]), mid_th = (min_th + max_th)/2.;
	bool progress_msg = false;
...
// an alpha-MLE point if x(1)-1< tha <x(n), or an open-end if tha = x(1)-1
		...
		else  if ( (model==M1 && tha > Xs[0] - zero_eq) || (model==M2 && tha > Xs[0]-1.) ) {
		...
		for (k=k_1;k<=kb;k++) {
			...
			if(output) if(met==GEO) if(!progress_msg) if(th>mid_th) {
				Rcout << "  50%...";  Rflush();  progress_msg = true;  }
