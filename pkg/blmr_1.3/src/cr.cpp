...
// get theta-boundaries of confidence region(s)
	if(output) if(met==GEO) { Rcout << "               " << _("getting theta-boundaries...") << endl << endl; Rflush();}
	METHOD  met2;
...
// store boundary values in an  N x 3  array	
	...
	if (output) {
		...
		Rcout << endl << setw(10) << "theta" << setw(13) << "min.-alpha" << setw(16) 
				<< "max.-alpha" << endl;
		Rflush();
	}
...
// confidence region ends at "thb" analagously to its start at "tha", that is with a vertical line
...
{
//Rcout << "ahigh met th  " << met << " " << th << endl;
	bool th_ex;
	if ( (model==M1 && th<=Xs[0]) || Xs[ns-1]<=th) th_ex=true; else th_ex=false;
//Rcout << "th_ex " << th_ex << endl;
	double amle;
	if (th_ex) {
		if(model==M1) {
			double ff = sxx-2*sx1*th+s11*th*th, fy = yx-th*y1, f1 = sx1-th*s11;
...
//Rcout << "high_alpha " << high_alpha << endl;
	return high_alpha;
}
