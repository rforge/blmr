...
// "const" version of above general bisection routine
{
	double  x1= a, x2= b, f1 = (this->*fn)(x1,k) - value,  f2 = (this->*fn)(x2,k) - value;
	if (f1*f2>0 || f1==f2 || isnan(f1*f2)) {
		Rcout << _("'bisect' to find x such that f(x)=0 starting from input values:") << endl;
...
double Cblmr::integrate(const double x1, const double x2, double (Cblmr::*fn)(double, int) const, const int k, double *const er)  const
...
{
	if (er != 0) *er=0.;
	if ( fabs(x1-x2) < zero_eq) return 0.;

	const double a= min(x1,x2), b= max(x1,x2), abs_error= acc_sl_abs/2/n, rel_error= acc_sl_rel/2;
...
// initial estimate
	...
	while (intervals < 1025 && error > abs_error  &&  fabs(error/estimate) > rel_error) {
...
double Cblmr::integrate2(const double par_a, const double par_b, double (Cblmr::*fn)(double, double*), double *const err2)
...
{
  if (err2 != 0) *err2=0.;
  if ( fabs(par_b - par_a) < zero_eq )  return 0.;

  const double  a =min(par_a,par_b), b =max(par_a,par_b), epsrel = acc_sl_rel;
  double  epsabs = acc_sl_abs;
  if (!variance_unknown) epsabs /= lambda;
  double  ferr =0, er=0., *const per=&er;
...
// at this point, failed to converge by  21-43-87  composite Gauss-Kronrod-Patterson
// so switch to adaptive quadrature, integrate subintervals by 21-point Gauss-Kronrod
...
// repeat until error fits condition or until reached maximum subintervals
  ...
  while( numi < limit  &&  err > epsabs  &&  err > epsrel*fabs(intgsum) ) {
	  ...
	  intg[kmax] = qgk21(ai,bi, fn, per);
	  errs[kmax] =*per;

	  ai=bi; 
	  bi=ablist[kmax+2];
	  intg[kmax+1] = qgk21(ai,bi, fn, per);
	  errs[kmax+1] =*per;
  ...
  Rf_warning( _("'integrate2' did not reach tolerance after highest-order composite rule nor highest-order adaptive rule") );
stop("debug");
...
double Cblmr::qgk21(const double a, const double b, double (Cblmr::*fn)(double, double*), double *const err2)
...
rescale_error (double err, const double result_abs, const double result_asc)
{
  ...
  if (result_asc != 0 && err != 0) {
        const double scale= pow((200*err/result_asc), 1.5);
        if(scale < 1)  err= result_asc*scale; else  err= result_asc;
      }

  if (result_abs > DBL_MIN / (50*DBL_EPSILON))  {
      const double min_err = 50 * DBL_EPSILON * result_abs ;
