...
int Cblmr::cr(const METHOD met, const double inc, const bool output, double *const bounds)
//
...
// get theta-boundaries of confidence region(s)
	...
	double *const  tmp= new (nothrow) double[2*n_d];
	if(tmp==NULL) {
	...
	const double th_min= x_d[0]-1, th_max= x_d[n_d-1]+1;
	if(th_bds[0]== -inf)  th_bds[0]= th_min;
	if(th_bds[2*numr-1]== +inf)  th_bds[2*numr-1]= th_max;
	for(i=0;i<2*numr;i++) {
		if( fabs(th_bds[i]-x_d[0]) < zero_eq )  th_bds[i]= x_d[0];
		if( fabs(th_bds[i]-x_d[n_d-1]) < zero_eq )  th_bds[i]= x_d[n_d-1];
	}
...
// store boundary values in an  N x 3  array
	...
	const int Nmax = width/inc + 1 + n_d + 2*numr + 2;
	...
	if( trivial && !isnan(thmle) && thmle!=x_d[0] )  {
		...
		*(bds+N*3+0) = thmle; *(bds+N*3+1) = amle; *(bds+N*3+2) = amle; N++;
		...
		if(output) if(met==GEO) { 
			...
			if(Model==M1)  min_th = max(th_bds[0],x_d[0]);  else  min_th = th_bds[0];
			max_th = min(th_bds[2*numr-1],x_d[n_d-1]);
...
// otherwise an alpha-MLE point
			...
			else  if( tha == x_d[n_d-1] ) {
				th = x_d[n_d-1] + fabs(rel_print_eps*x_d[n_d-1]);
				high_a = ahigh(met,th);
				*(bds+N*3+0) =tha; *(bds+N*3+1) =high_a; *(bds+N*3+2) =high_a; N++; 
				*(bds+N*3+0) =th;  *(bds+N*3+1) =a_sl(met,th,-1); *(bds+N*3+2) =a_sl(met,th,1); N++; 
			}
			else  if( Model==M1  &&  tha == x_d[0] ) {
				th = x_d[0] + fabs(rel_print_eps*x_d[0]);
				high_a = ahigh(met,th);
				*(bds+N*3+0) =tha; *(bds+N*3+1) =high_a; *(bds+N*3+2) =high_a; N++;
				*(bds+N*3+0) =th; *(bds+N*3+1) =a_sl(met,th,-1); *(bds+N*3+2) =a_sl(met,th,1); N++; 
			}
...
// find (theta,alpha)-boundaries at increments of theta and at datapoints
			...
			int ka= -1, kb= -1; 
			while (x_d[ka+1]<=tha && ka<n_d-1) ka++;
			while (x_d[kb+1]<=thb && kb<n_d-1) kb++;


			for (k=ka;k<=kb;k++) {

				double th1i; 
				if( k < 0 ) th1i= tha; else th1i= max(tha, x_d[k]);
...
// confidence region boundary is discontinuous at x(n), and at x(1) in M1,
				...
				if(k< n_d-1) th2= min(x_d[k+1],thb); else th2= thb;
...
double Cblmr::a_sl(const METHOD met, const double th, const int high_low)
...
// if met=GEO use AF alpha as first guess, then grid search + bisection
{
	...
	if (met==AF)  {
		...
		const double a_geo =bisect(guess,guess2,&Cblmr::sl_a,1,SL,-acc_yb);
		return a_geo;
	}
}
...
double Cblmr::a_af(const double th, const int high_low)
...
// if 'high_low' > 0 return high value, if 'high_low' < 0 return low value
{
	...
	if (th != prev_th) {
		...
		bool th_ex;
		if ( (Model==M1 && th<=x_d[0]) || x_d[n_d-1]<=th) th_ex=true; else th_ex=false;
		...
		if (th_ex) {
			...
			int k=0; while (k<n_d && x_d[k]<th) k++;
...
double Cblmr::ahigh(const METHOD met, const double th)
...
double Cblmr::sl_a(const double alpha, const int k)
