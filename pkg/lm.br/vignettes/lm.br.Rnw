...
a known threshold level.  Inferences about a parameter use
...
a formula, which \texttt{lm.br} implements, to rapidly evaluate these tests.\\
...
the Normal approximations of binary random variables.  And these
tests simplify for a postulate changepoint value
outside the range of  $x$-values, 
giving exact significance levels for the hypothesis of a single 
...
Simulations and examples demonstrate all the theory.
...
As an example, a broken-line model could fit drinking-and-driving survey 
results.
...
The data input, and commands to get confidence intervals, are
...
\section{Summary}
If a broken line with Normal errors represents the relationship between a factor and responses, then \texttt{lm.br} solves the inference step for the changepoint.  This package uses the technique of conditional inference to allow for the other, unknown terms of the model.  
