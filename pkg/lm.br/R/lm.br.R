...
lm.br  <- function( formula, type ="LL", data, subset, weights, inverse =FALSE, var.known =FALSE, 
						offset, contrasts =NULL, na.action, ... ) 
{
    call <- match.call()
	...
	if( NCOL(y) > 1 )  stop( "mutiple response vectors not supported" )

    w <- model.weights(mf)
	w_ <- w
	if( is.vector(w) )  { if(inverse)
		{ if(any(w==0)) stop( "zero variances not allowed" )  else  w <- 1/w }
	}  else
	if( is.matrix(w) )  {
		n <- NROW(y)
		if( any( dim(w) != c(n, n) ) )  stop( "dim('weights') invalid" ) 
		eW <- eigen(w, TRUE)
		d <- eW$values
		...
		if (!is.null(offset)) offset <- as.vector( A %*% as.matrix(offset) )
	}
...
# 'xb' is the 'x' input matrix, but with two vectors for 'x1' before and after changepoint,
#  and zeroes for columns that were linearly dependent
		nx <- ncol(x)
		nxb <- ncol(x)+1
		xb <- matrix( 0, nrow(x), nxb )
		xb[,1] <- x[,1]
		for(i in x1c:nx) xb[,i+1] <- if( is.na(z$coef[i]) )  0  else  x[,i]

		bnm <- paste( " ", x1nm, "< theta" )	
		bpnm <- paste( " ", x1nm, "> theta" )
		rownames(xb) <- rownames(x)
		colnames(xb) <-  if( xint ) 
				{ if( x1c < nx )  c("1-vector", bnm, bpnm, dn[(x1c+1):nx])  else  c("1-vector", bnm, bpnm) }
			else  
				{ if( x1c < nx )  c(bnm, bpnm, dn[(x1c+1):nx])  else  c(bnm, bpnm) }


#  construct C++ object
#  re-order for 'x1' non-decreasing,  drop rows with  w = 0,  drop columns not independent
		x_ <- x[ order(x1), , drop = FALSE]
		y_ <- y
		if(!is.null(offset)) y_ <- y_ - as.vector(model.offset(mf))
		y_ <- y_[ order(x1) ]
		if( !is.null(w_) )  w_ <- if(is.matrix(w_))  w_[ order(x1), order(x1) ]  else  w_[ order(x1) ]
...
#  loop to drop columns that are not independent at some changepoint value
#  if x-matrix is dependent at  changepoint = 'th'  then  Qf(th)=0  on lower rows, f(th)= max(x1-th,0)
		x_dep <- TRUE
		while( x_dep ) {
			...
			obj <- if( is.null(w) ) 
						...
						new( Cpp_Clmbr, y_, x_, type, as.matrix(w_), inverse, var.known )

			thQfmin <- obj$param()[5]
			xb[ ,x1c] <- if( type=='TL' )  0  else  pmin( x1 - thQfmin, 0 )
			xb[ ,x1c+1] <- if( type=='LT' )  0  else  pmax( x1 - thQfmin, 0 )
			...
			if( xbr < nb )  {
				...
				for(i in 1:nx) ok[i] <- !is.na(z$coef[i+1])
...
# output
		...
		if( is.nan( par[1] ) )  {	# case of perfect line
			...
			if( is.infinite(par[1]) )  {
				...
				xb[ ,x1c]  <- if( type=='TL' )  0  else  pmin( x1 - par[1], 0 )
				xb[ ,x1c+1] <- if( type=='LT' )  0  else  pmax( x1 - par[1], 0 )
			}
		}
...
# accessor functions
		...
		z$sl <- function( theta0, alpha0 =NULL,  method ="clr", accuracy =0.001, verbose =NULL ) 
		...
		z$sety <- function( rWy )  {
			if( NCOL(rWy) > 1 | !is.numeric(rWy) )  stop( "'rWy' must be a numeric vector" )
			rWy <- drop(rWy)
...
#  output list
		p <-  if( is.nan( par[1] ) )  z$rank  else  z$rank + 1		# 'theta' is a parameter
...
print.lm.br  <-  function ( x, digits = max(3L, getOption("digits") - 3L), ... ) 
{
    ...
    if (length(coef(x))  && !is.na(x$coef[1])) {
		cat( "Significance Level of hypothesis \"no changepoint\" versus h. \"one changepoint\"\n" )
		cat("  ")
		mx1 <- max( x$x1 )
		mn1 <- min( x$x1 )
