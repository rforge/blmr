...
the Normal approximation of a binary random variable.  And these exact
significance tests simplify for a postulate changepoint value
outside the range of  $x$-values \citep{knowles+siegmund:1989}, 
giving an exact significance test for the hypothesis of a single 
line versus a broken line.\\
...
\multicolumn{6}{c}{\textsf{Table: Coverage frequencies of the 95\% confidence interval on 100 models}} 
\rule[-2ex]{0pt}{0ex}\\
...
Formulae created the 100 arbitrary models using $U \sim Uniform(0,1)$:\\
\begin{tabular}{cccc}
\rule[-2ex]{0pt}{5.5ex}
...
results.
Yearly surveys, taken in different months some years, were adjusted by a seasonal index based on monthly
