...
outside of  $[ x_{1}, x_{n} ]$  \citep{knowles+siegmund:1989}.\\
...
and outside $[ x_{1}, x_{n} ]$, in model (1).  The
...
\section{Remarks}
If a broken line with Normal errors represents the relationship between a factor and responses, \texttt{lm.br} solves the inference step for the changepoint.  Fitting a broken line can reveal the plausible interval for a changepoint, although practical cause-effect relations usually have 
