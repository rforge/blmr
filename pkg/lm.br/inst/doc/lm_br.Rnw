...
%\VignetteIndexEntry{lm_br}
%\VignettePackage{lm.br}
...
demonstrate its use and note some properties 
...
changepoint.  Exactly, the broken-line models are
...
and $a_{+}=max(a,0)$.  $\theta$, $\alpha$, $\beta$, $\beta{}'$,
$\sigma$ are
unknown but $\Sigma$ is known.  Model (2) is a threshold model,
while model (3) would apply for a known threshold level.\\
...
model.  The set of postulate values such that  $SL > 1 - \alpha$
is a $100\alpha\%$ confidence region.\\
...
small samples.  The familiar Student's $t$, for example, is the
...
\texttt{lm.br} augments this theory.  Their method derives an
exact significance test for $(\theta, \alpha)$ in model (1).  The
theory adapts for the case  $\sigma$  known.  And these exact
significance tests simplify for a postulate changepoint value
outside  $[ x_{1}, x_{n} ]$  \citep{knowles+siegmund:1989}.\\

Approximate-F (AF) is another inference method that is common for
nonlinear regression, but it is not exact.  The AF method
estimates the distribution of a likelihood-ratio statistic by its
asymptotic O2 distribution, with partial conditioning on a
sufficient statistic for the variance.  See 
\citet[ch.24]{draper+smith:1998}.
...
and with seasonal adjustment, and the exact 95\% confidence region
...
(circles) and log-odds with seasonal adjustment (squares) versus 
year, and the exact 95\% confidence region for the 
changepoint $(\theta,\alpha)$.}
\end{center}
\end{figure}
...
The wide difference in confidence intervals here is due to
plateaus in the significance levels on end-intervals.  Both the
CLR and the AF methods give a constant significance level for all
postulate values $\theta_{0}$ on $[ x_{1}, x_{2} ]$, on $[ x_{n-
1}, x_{n} ]$, and outside $[ x_{1}, x_{n} ]$, in model (1).  The
inference assumes that any line slope is possible, extending to an
instantaneous drop near Dec. 1998 in this example.

\subsection{Multivariate Regression}
\texttt{lm.br} can test for a changepoint in multivariate
regression.  \texttt{lm.br}  tests for a coefficient change in the
...
against weight and horsepower for 38 cars, 1978-79 models.  One 
of \textsf{R}'s included datasets is the ratings for 32 cars, 
1973-74 models.  Analysis of this latter dataset by conditional 
likelihood-ratio using \texttt{lm.br} also shows some evidence 
for a changepoint:
<<>>=
lm.br( mpg ~ wt + hp, data = mtcars )
@
\texttt{lm.br} applies a canonical transformation for multivariate regression 
