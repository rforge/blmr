...
## test 2 random models for each type of model
   ...
   for(nmodel in c(-3,-2,1,2,3) )  { for(vk in 0:1)
      ...
      ntests <- ntests + 1
      tcurrent <- Sys.time()
      dtime <- round( as.numeric( difftime(tcurrent,tstart,units="mins") ), 0 )
...
# construct an 'lm.br' object with arbitrary 'y' values
      ...
      for( i in 1:N ) {
         ...
         if(i/1000 - floor(i/1000) == 0) {
            cat( format(i,width=10),
               format( countCLR/i, digits=4, nsmall=4, 
                          width=22 ),
               format( countAF/i, digits=4, nsmall=4, 
                          width=22),"\n")
            flush.console()
         }
      }
   ...
   if( !passed && ntrials==3 ) stop("fails interval coverage test")
   if( cicov > .97) 
      warning( gettextf("conf. interval coverage frequency  %d%%", 
   ...
   if( !passed && ntrials==3 ) stop("fails region coverage test")
   if( crcov > .97) 
      warning( gettextf("conf. region coverage frequency  %d%%", 
