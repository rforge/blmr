...
RCPP_MODULE(Clmbr){
	...
	NumericMatrix (Clmbr::*cr4) (double, string, double, bool) = &Clmbr::crR ;
	void (Clmbr::*cr5) (void) = &Clmbr::crR ;


	class_<Clmbr>( "Cpp_Clmbr" )
	
	    // expose the constructors
	    .constructor< NumericVector, NumericMatrix, string, NumericMatrix, bool, bool >()    
	    .constructor< NumericVector, NumericMatrix, string, bool >()
		...
		.method( "cr", cr4 , "confidence region for (theta,alpha) by specified method and increment, return matrix" )
		.method( "cr", cr5 , "confidence region for (theta,alpha) at default parameter values" )
