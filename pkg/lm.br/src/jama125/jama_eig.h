   ...
   //  Fortran subroutine in EISPACK.
      ...
      for (int l = 0; l < n; l++) {
         ...
         // Find small subdiagonal element
		  ...
		  double tmarc = fabs(d[l]) + fabs(e[l]);
		  if (tmarc > tst1) tst1 = tmarc;
   ...
   void hqr2 () {
         ...
         // No convergence yet
            ...
            for (int k = m; k <= n-1; k++) {
               ...
               if (s != 0) {
				  ...
				  int imarc2 = n;
				  if ( k+3 < imarc2 ) imarc2 = k+3;
                  for (int i = 0; i <= imarc2; i++) {
                     p = x * H[i][k] + y * H[i][k+1];
                     if (notlast) {
                        p = p + z * H[i][k+2];
                        H[i][k+2] = H[i][k+2] - p * r;
         ...
         // Complex vector
            ...
            for (int i = n-2; i >= 0; i--) {
               ...
               if (e[i] < 0.0) {
				  ...
				  double tmarc2 = fabs(H[i][n-1]);
				  if ( fabs(H[i][n]) > tmarc2 ) tmarc2 = fabs(H[i][n]);
                  t = tmarc2;
                  if ((eps * t) * t > 1) {
                     for (int j = i; j <= n; j++) {
                        H[j][n-1] = H[j][n-1] / t;
      ...
      for (int j = nn-1; j >= low; j--) {
         for (int i = low; i <= high; i++) {
            z = 0.0;
			int imarc3 = high;
			if (j < imarc3) imarc3 = j;
            for (int k = low; k <= imarc3; k++) {
               z = z + V[i][k] * H[k][j];
            }
            V[i][j] = z;
         }
