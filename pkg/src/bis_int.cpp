...
// if  crit < 0  find x such that value - crit < fn(x) < value
{
	...
	if (f1*f2>0 || f1==f2 || isnan(f1*f2)) {
		Rcout << "bisect cannot resolve x1, x2, f(x1), f(x2) =  " <<
			x1 << " " << x2 << " " << f1 << " " << f2 << endl;
		return und;
	}
	int iteration=0;
	...
	if(iteration==50)  Rcout << "bisect failed to reach tolerance after 50 iterations" << endl;
...
// if  crit < 0  find x such that SL - crit < sl(x) < SL
{
	...
	if (p12>0 || f1==f2 || fabs(p12)>1 || isnan(p12)) {
		Rcout << "bisect_sl cannot resolve x1, x2, sl(x1), sl(x2) =  " <<
			x1 << " " << x2 << " " << f1+SL << " " << f2+SL << endl;
		return und;
	}
	int iteration=0;
	while (  fabs(x1-x2) > fabs(crit)  && (iteration<50) ) {
	...
	if(iteration==50)  Rcout << "bisect_sl failed to reach tolerance after 50 iterations" << endl;
	if (crit<0) { if (f1 <= 0) return x1; else return x2; }
		else  { if (f1 >= 0) return x1; else return x2; }
}
...
// initial estimate
	...
	while (error > abs_error  &&  fabs(error/estimate) > rel_error) {
...
//  estimate integral of fn(x)dx for  x = x1 to x2  by Gauss-Konrod 21, 43 or 87 point rule, or adaptive
...
//  the average error estimate of the integrand to the error estimate of this numeric integral.
{
...
// repeat until error fits condition or until reached maximum subintervals
  ...
  Rcout << "integrate2 failed to reach tolerance with highest-order rule" << endl;
