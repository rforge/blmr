...
double Cblmr::mu_by_tau(double th, int k)
{
	...
	if (fabs(th-th0) < zero_eq || k==gk0)  return  _copysign( inf, mu );

	double pisq = 1. - z*z - wzr*wzr/r;
	if ( pisq < zero_eq )  return  _copysign( inf, mu );

	double OMsq = dgsq(th,k) - dro*dro/r;

	if ( OMsq < zero_eq )  return  _copysign( inf, mu );
...
double Cblmr::mu_by_tau_sq(double th, int k)
{
	...
	if (th < th0)  sign_dro = _chgsign(sign_dro);

	double sign_mu = sign_dro*zwr;
	
	if (Xs[k]<th0+zero_eq && th0<Xs[k+1]+zero_eq) return  _copysign( inf, sign_mu );

	double musq = drosq*zwr*zwr/r/r;

	double pisq = zz - wzr*wzr/r;

	if ( pisq < zero_eq )  return  _copysign( inf, sign_mu );

	double OMsq = dgsq(th,k) - drosq/r;

	if ( OMsq < zero_eq )  return  _copysign( inf, sign_mu );

	double tausq =  OMsq * pisq ;

	return  _copysign( musq/tausq, sign_mu ) ;
}
